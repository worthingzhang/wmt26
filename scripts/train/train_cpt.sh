#!/usr/bin/env bash
# Train CPT (continued pre-training) using LlamaFactory in thunlp_opd.
# Usage:
...
Train CPT (continued pre-training) with LlamaFactory.

Required:
  --run-id ID                  Unique run ID.
  --input-model PATH           Path to input model (base or checkpoint).
...
if [[ ! -d "${LLAMAFACTORY_DIR}" ]]; then
    ...
    echo "Please clone thunlp_opd repo first."
    exit 1
fi
...
# Copy configs to run dir for reproducibility
cp "${DATA_CONFIG_ABS}" "${RUN_DIR}/data_config.yaml"
cp "${TRAIN_CONFIG_ABS}" "${RUN_DIR}/train_config.yaml"

# Build temporary config with placeholders replaced
...
echo "${RUN_ID},cpt,${INPUT_MODEL_ABS},,${OUTPUT_MODEL_ABS},${DATA_CONFIG_ABS},${TRAIN_CONFIG_ABS},${RUN_DIR},running,started at $(date -Iseconds)" >> "${TRAIN_REGISTRY}"

# Run training
cd "${LLAMAFACTORY_DIR}"
# TODO: verify exact llamafactory-cli invocation in thunlp_opd/LlamaFactory
...
if [[ ${TRAIN_STATUS} -eq 0 && -d "${OUTPUT_MODEL_ABS}" ]]; then
    python "${REGISTER_MODEL_PY}" \
        ...
        --notes "CPT run ${RUN_ID}"
fi

echo "${RUN_ID},cpt,${INPUT_MODEL_ABS},,${OUTPUT_MODEL_ABS},${DATA_CONFIG_ABS},${TRAIN_CONFIG_ABS},${RUN_DIR},${STATUS},finished at $(date -Iseconds)" >> "${TRAIN_REGISTRY}"

echo "CPT training finished with status: ${STATUS}"
exit ${TRAIN_STATUS}
